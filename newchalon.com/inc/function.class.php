...
//取出产品目录所有一级目录    返回变量数组 引用时需要赋值 再输出
function getproductdir1(){
global	 $objDB;//函数调用 声明全局变量
$strSQL="SELECT id_proddir,name,intro FROM productdir WHERE level='1' and dele='1' and fatherid=0 order by ordernum asc";
$objDB->Execute($strSQL);
...
//取出区块layoutpic指定id的所有图片
function getlayoutallpic($lid){
global	 $objDB;//函数调用 声明全局变量
$strSQL = "select opicname as pic,url from layoutpic  where id_layout ='".$lid."' order by  id_layoutpic asc" ;
$objDB->Execute($strSQL);
$arrallpic=$objDB->GetRows();
return $arrallpic;
}
