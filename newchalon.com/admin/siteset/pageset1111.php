...
//交换图片位置
	if(isset($_GET["action"]) && $_GET["action"]=="picswitch"){
		$strSQL="UPDATE pagesetpic SET ordernum=".$_GET[swtorder]." WHERE id_pagesetpic=".$_GET[id]."";
		$objDB->Execute($strSQL);
		$strSQL="UPDATE pagesetpic SET ordernum=".$_GET[order]." WHERE id_pagesetpic=".$_GET[swtid]."";	
		$objDB->Execute($strSQL);
	    header('Location:pageset.php?03'.$_GET[id_pageset]);ob_end_flush(); exit();//跳转
	}		
...
<div id="lineout">
   ...
   $strSQL="SELECT opicname,id_pagesetpic,type,title,ordernum FROM pagesetpic WHERE id_pageset=$onenewsid order by  ordernum asc";
   $objDB->Execute($strSQL);
          ...
          <td align="center"><table width="93%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <table width="100%" border="0" cellpadding="4" cellspacing="0" class="txt">
                  ...
                  <? if($onenewsid!='2' && $onenewsid!='19' && $onenewsid!='7' && $onenewsid!='20'){//排除版面内容?>
                        <tr bgcolor="#FFFFFF">
                    <td valign="top" bgcolor="#FFFFFF">信息内容</td>
...
?>
<div class="box floatLeft" style="position:relative; background-color:#154f79"><a href="javascript:void(0)"  onclick="$.wholepop.dialogClass ='style_1';poppic('/upload/layout/<?php echo $onenewspicinfo[$i][opicname];?>','图片预览及地址复制','<?php echo $onenewspicinfo[$i][id_pagesetpic];?>');" ><img src="/upload/layout/<?php echo $onenewspicinfo[$i][opicname];?>" width="50" height="50" border="0" /></a>

<span style="float:left;height:25px; width:50px; cursor:pointer; color:#ccc;overflow:hidden" onclick="$.wholepop.dialogClass ='style_1';poppic('<?php echo $siteurl;?>/upload/layout/<?php echo $onenewspicinfo[$i][opicname];?>','图片预览及地址复制','<?php echo $onenewspicinfo[$i][id_pagesetpic];?>');"><?php if($onenewspicinfo[$i][title]!=''){echo $onenewspicinfo[$i][title];}else{echo '无标题';}?></span>
<div style="clear:both"></div>
<span class="txt_underpic" onclick="delpic(<?php echo $onenewspicinfo[$i][id_pagesetpic];?>);" style="cursor:pointer;color:#FFF">删除</span>


<?
$picnum=sizeof($onenewspicinfo);
if($i==0){
	$pic_swtid=$onenewspicinfo[$picnum-1][id_pagesetpic];
	$pic_swtorder=$onenewspicinfo[$picnum-1][ordernum];
}else{
	$pic_swtid=$onenewspicinfo[$i-1][id_pagesetpic];
	$pic_swtorder=$onenewspicinfo[$i-1][ordernum];
}

if($i==($picnum-1)){
	$pic_swtid2=$onenewspicinfo[0][id_pagesetpic];
	$pic_swtorder2=$onenewspicinfo[0][ordernum];
}else{
	$pic_swtid2=$onenewspicinfo[$i+1][id_pagesetpic];
	$pic_swtorder2=$onenewspicinfo[$i+1][ordernum];
}

?>
  <a href='pageset.php?action=picswitch&id=<?=$onenewspicinfo[$i][id_pagesetpic];?>&order=<?=$onenewspicinfo[$i][ordernum];?>&swtid=<?=$pic_swtid;?>&swtorder=<?=$pic_swtorder;?>&id_pageset=<?=$onenewsinfo[id_pageset];?>'>
  <img src="../inc/pics/icon_006.gif" width="10" height="10" border="0" style="position:absolute;bottom:7px;left:0px;" />
  </a>
  
  <a href='pageset.php?action=picswitch&id=<?=$onenewspicinfo[$i][id_pagesetpic];?>&order=<?=$onenewspicinfo[$i][ordernum];?>&swtid=<?=$pic_swtid2;?>&swtorder=<?=$pic_swtorder2;?>&id_pageset=<?=$onenewsinfo[id_pageset];?>'>
  <img src="../inc/pics/icon_005.gif" width="10" height="10" border="0" style="position:absolute;bottom:7px;right:0px;" />
  </a>
